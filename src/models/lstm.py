...
# 체크포인트 저장 디렉토리 생성
os.makedirs('outputs/checkpoints/lstm', exist_ok=True)

class TemperatureLSTM(nn.Module):
    def __init__(self, input_size=70, num_layers=2):
        super(TemperatureLSTM, self).__init__()
        
        self.lstm = nn.LSTM(input_size, 100, num_layers, batch_first=True, dropout=0.3)
        self.fc = nn.Linear(100, 858)
...
def train_model_2(model, train_dataloader, valid_dataloader, num_epochs=300, resume_path=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save({
                'model_state_dict': model.state_dict(),
            }, 'outputs/checkpoints/lstm/LSTM1_100_test.pth')
...
def train_model(model, train_dataloader, valid_dataloader,num_epochs=1000, resume_path=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if best_val_loss - avg_val_loss > threshold:  # threshold 이상 개선된 경우
            ...
            }, 'outputs/checkpoints/lstm/LSTM1_100.pth')
        else:
    ...
    plt.title('Training and Validation Loss')
    plt.legend()
    plt.savefig('outputs/checkpoints/lstm/loss_plot.png')
    ...
    plt.legend()
    plt.savefig('outputs/checkpoints/lstm/r2_plot.png')
