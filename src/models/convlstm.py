...
batch_size = 64
num_epochs = 100 
os.makedirs('outputs/checkpoints/convlstm', exist_ok=True)
...
def train_model_2(model, train_dataloader, valid_dataloader, num_epochs=300, resume_path=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save({
                'model_state_dict': model.state_dict(),
            }, 'outputs/checkpoints/convlstm/base_model.pth')
...
def train_model(model, train_dataloader, valid_dataloader,num_epochs=1000, resume_path=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if best_val_loss - avg_val_loss > threshold:  # threshold 이상 개선된 경우
            ...
            }, 'outputs/checkpoints/convlstm/test_model.pth')
        else:
    ...
    plt.title('Training and Validation Loss')
    plt.legend()
    plt.savefig('outputs/checkpoints/convlstm/loss_plot.png')
    ...
    plt.legend()
    plt.savefig('outputs/checkpoints/convlstm/r2_plot.png')
